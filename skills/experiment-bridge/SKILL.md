...
4. **Self-review before deploying:**
   ...
   - **⚠️ Ground truth integrity check**: Is evaluation performed against the actual dataset labels (e.g., from the test split annotation files), and NOT against predictions or outputs from any baseline or backbone model? Using backbone outputs as pseudo-labels is result fraud — verify the ground truth source explicitly in the eval script.
...
mcp__codex__codex:
  ...
  prompt: |
    ...
    5. **Ground truth source**: Is the evaluation comparing model predictions against the official dataset ground truth labels (annotation files / test split labels)? Flag immediately if the code compares against backbone/baseline model outputs instead — this constitutes result fraud.
    ...
    Ground truth contamination (item 5) must always be rated CRITICAL.
```
...
4. **Verify ground truth source** — confirm each result file records the ground truth path or source used during evaluation. If any result was computed against backbone/baseline outputs rather than dataset labels, discard and re-run with the correct ground truth before proceeding.
...
- **⚠️ Ground truth integrity (critical).** Evaluation MUST always compare model predictions against the official dataset ground truth labels (annotation files, test split labels provided by the dataset). NEVER use backbone model outputs, baseline predictions, or any other model's outputs as ground truth labels — this constitutes result fraud and invalidates all conclusions. Verify the ground truth source at Phase 2 (self-review), Phase 2.5 (code review), and Phase 5 (result collection). If contamination is detected at any stage, stop, fix, and re-run.
